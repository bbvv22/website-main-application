...
const Header = () => {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto py-4"> {/* Removed px-8 */}
          <div className="flex items-center justify-between px-8"> {/* Added px-8 */}
            {/* Logo */}
            <Link to="/">
            ...
            <div className="hidden md:flex items-center space-x-8">
              ...
              <motion.button 
                ...
                className="w-8 h-8 flex items-center justify-center text-dwapor-beige hover:text-dwapor-gold transition-colors relative"
              >
              ...
              <div className="relative">
                <motion.button 
                  ...
                  className="w-8 h-8 flex items-center justify-center text-dwapor-beige hover:text-dwapor-gold transition-colors"
                ...
                {isAuthenticated && isUserMenuOpen && (
                  ...
                  >
                    <div className="px-4 py-2 border-b border-dwapor-soft-gray/20">
                      <p className="text-dwapor-amber font-medium text-sm">Hello {user?.firstName}</p>
                    </div>
            ...
            {/* Mobile Icons */}
            <div className="flex items-center md:hidden"> {/* Removed space-x-4 */}
              {/* Cart */}
              <motion.button 
                whileHover={{ scale: 1.1, color: '#c19a6b' }}
                whileTap={{ scale: 0.95 }}
                onClick={() => setIsCartOpen(true)}
                className="text-dwapor-beige hover:text-dwapor-gold transition-colors relative" // Removed ml-auto
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                </svg>
              ...
              {/* User Account */}
              <div className="relative ml-2"> {/* Added ml-2 */}
                <motion.button 
                  whileHover={{ scale: 1.1, color: '#c19a6b' }}
                  whileTap={{ scale: 0.95 }}
                  onClick={handleAuthAction}
              ...
              <motion.button
                ...
                className="ml-2 text-dwapor-beige hover:text-dwapor-gold transition-colors" // Added ml-2
              >
          ...
          <motion.div
            ...
            className="md:hidden absolute top-full left-0 right-0 bg-dwapor-museum z-40 overflow-hidden shadow-lg" // Added absolute, top-full, left-0, right-0, bg-dwapor-museum, z-40, shadow-lg
          >
            <nav className="px-8 py-6 space-y-6"> {/* Adjusted padding */}
              {navItems.map((item) => (
