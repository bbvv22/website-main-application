...
export const CartProvider = ({ children }) => {
  ...
  const loadCartFromBackend = async (userId) => {
    try {
      setLoading(true);
      const { data: backendCart } = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/cart/${userId}`);
    ...
    const syncCart = async () => {
      if (user && isAuthenticated) {
        ...
        if(localCart.length > 0){
            const { data: backendCart } = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/cart/${user.id}`);
            ...
            localCart.forEach(localItem => {
              ...
              if (!backendItem) {
                ...
                axios.post(`${process.env.REACT_APP_API_BASE_URL}/cart/${user.id}`, {
                  productId: localItem.id,
                  quantity: localItem.quantity,
                  size: localItem.selectedSize,
                  color: localItem.selectedColor,
  ...
  const addToCart = async (product, selectedSize, selectedColor, quantity = 1) => {
    ...
    try {
      ...
      if (isAuthenticated && user?.id) {
        const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/cart/${user.id}`, {
  ...
  const removeFromCart = async (cartId) => {
    if (user) {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/cart/${user.id}/${cartId}`);
  ...
  const updateQuantity = async (cartId, newQuantity) => {
    if (user) {
      const { data: updatedCartItem } = await axios.put(`${process.env.REACT_APP_API_BASE_URL}/cart/${user.id}/${cartId}`, { quantity: newQuantity });
      setCartItems(prevItems =>
        prevItems.map(item =>
          item.id === cartId
  ...
  const clearCart = async () => {
    if (user) {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/cart/${user.id}`);
    }
    setCartItems([]);
  };
