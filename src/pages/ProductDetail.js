...
import { getProductFeatures } from '../data/productData';
...
const ProductDetail = () => {
  ...
  const handleAddReview = async (review) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/reviews`, {
        ...review,
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/reviews/${product.id}`);
          ...
          >
            ...
            <div className="mt-8 p-6 border border-dwapor-soft-gray/20 rounded-lg bg-white">
              ...
              <form onSubmit={async (e) => { 
                ...
                if (!data.question.trim() || data.question.length < 10) {
                ...
                try {
                  // ✅ FIX: Use 'id' from useParams(), not 'product.id'
                  await axios.post(`${process.env.REACT_APP_API_BASE_URL}/contact`, { 
                    ...data, 
                    productId: id,  // ✅ FIXED: Using id from useParams()
                    type: 'product_question' 
                  });
                  showNotification('Thanks! We will get back to you shortly.');
                  e.target.reset();
                } catch (error) {
                  console.error('Error submitting question:', error);
                  showNotification('Failed to submit question. Please try again.');
                } finally {
                ...
                <div>
                  <input 
                    ...
                    required 
                    placeholder="Name" 
                    className={`w-full border rounded-md px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-dwapor-amber ${
                      questionErrors.name ? 'border-red-500' : 'border-dwapor-soft-gray/30'
                    }`}
                  />
                  {questionErrors.name && <p className="text-red-500 text-xs mt-1">{questionErrors.name}</p>}
                </div>
                ...
                <div>
                  <input 
                    ...
                    placeholder="Email" 
                    className={`w-full border rounded-md px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-dwapor-amber ${
                      questionErrors.email ? 'border-red-500' : 'border-dwapor-soft-gray/30'
                    }`}
                  />
                  {questionErrors.email && <p className="text-red-500 text-xs mt-1">{questionErrors.email}</p>}
                </div>
                
                {/* ✅ FIX: Use textarea instead of input, full width span */}
                <div className="md:col-span-2">
                  <textarea 
                    ...
                    rows={3}
                    className={`w-full border rounded-md px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-dwapor-amber ${
                      questionErrors.question ? 'border-red-500' : 'border-dwapor-soft-gray/30'
                    }`}
                  />
                  {questionErrors.question && <p className="text-red-500 text-xs mt-1">{questionErrors.question}</p>}
                </div>
                ...
                <div className="md:col-span-2">
                  <button 
                    ...
                    className="mt-2 px-6 py-3 bg-dwapor-amber text-dwapor-museum rounded-md text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-dwapor-gold transition-colors"
                  >
            ...
            <div>
              ...
              <p className="text-dwapor-soft-gray text-sm whitespace-pre-line">{product.care}</p>
            </div>
          </motion.div>
        </div>
